...
Integration layout (2026-09-09): install the separate media overlay into
`assets/legendary/`. Tower textures, the outdoor backdrop and ambient audio
now load from that directory; the entire directory is ignored by Git.
The `lib/` locations below describe the original donor integration only.
